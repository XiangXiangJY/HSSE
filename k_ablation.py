...
# Full K list (ablation will use prefixes of this list)
K_LIST_FULL = [5, 10, 15, 20, 30, 40, 50, 60, 70, 80]


# ======================================================
# Dataset-specific config (must match how PSL files were produced)
# ======================================================

def get_dataset_config(data_name: str, n_cells: int):
    """
    Return (pca_n_components, umap_dim, scales_full) for THIS dataset.
    You said:
      - human datasets use PCA_N_COMPONENTS=100
      - others use PCA_N_COMPONENTS=50
      - UMAP dim rule: <400 -> 15, 400-1200 -> 20, >1200 -> 50
      - scales: >1200 -> [5,23,46,72,100], else [5,14,25,37,50]
    """
    is_human = "human" in data_name.lower()
    pca_n_components = 100 if is_human else 50

    if n_cells > 1200:
        umap_dim = 50
        scales_full = [5, 23, 46, 72, 100]
    elif n_cells < 400:
        umap_dim = 15
        scales_full = [5, 14, 25, 37, 50]
    else:
        umap_dim = 20
        scales_full = [5, 14, 25, 37, 50]

    return pca_n_components, umap_dim, scales_full
...
def load_psl_features_block(psl_dir, data_name, n_cells, scale, k_local, dims_use=DIMS_USE):
    ...
    if len(matches) > 1:
        print(f"[warning] Multiple merged files found, using first: {matches[0]}")
    path = matches[0]
...
# ======================================================
# GBDT helpers (same as your original code)
# ======================================================
...
def run_gbdt_fivefold(X_feat, y, n_splits=5):
    ...
    for train_index, test_index in kf.split(X_feat):
        ...
        X_train_fold = X_feat[train_idx_fold]
        X_test_fold = X_feat[test_idx_fold]
...
# ======================================================
# K ablation runner (per dataset, scale fixed at FULL list)
# ======================================================

def run_one_dataset(DATA_NAME: str, out_dir: str = "./ablation_k_results"):
    ...
    pca_n_components, umap_dim, scales_full = get_dataset_config(DATA_NAME, n_cells)
    ...
    print("K_LIST_FULL:", K_LIST_FULL)
    print("ALPHA:", ALPHA)
    print("==============================\n")

    all_results = []

    # k ablation: use first t k's, scales always full
    for t in range(1, len(K_LIST_FULL) + 1):
        k_list_use = K_LIST_FULL[:t]

        print("\n----------------------------------")
        print(f"[Ablation] Using first {t} k(s): {k_list_use}")
        print(f"[Fixed]   Using all scales: {scales_full}")
        print("----------------------------------")

        feat_blocks = []
        for s in scales_full:
            for k_local in k_list_use:
        ...
        X_feat = np.concatenate(feat_blocks, axis=1)
        print("Final PSL feature matrix shape:", X_feat.shape)

        summary = run_gbdt_fivefold(X_feat, y, n_splits=5)
        ...
        row = {
            ...
            "umap_dim": umap_dim,
            "scales_fixed": ",".join(map(str, scales_full)),
            "k_list_use": ",".join(map(str, k_list_use)),
            "n_ks": t,
            "alpha": ALPHA,
        ...
        npy_path = os.path.join(out_dir, f"{DATA_NAME}_kAblation_first{t}.npy")
        np.save(npy_path, row, allow_pickle=True)

    csv_path = os.path.join(out_dir, f"{DATA_NAME}_k_ablation.csv")
...
def main():
    # Run one dataset at a time (HPC-friendly)
    datasets_to_run = [
        "GSE67835",
         #"GSE82187",
        #"GSE84133mouse1",
