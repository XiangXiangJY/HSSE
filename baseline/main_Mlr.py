...
# ======================================================
# Classifier selection (simple / linear models)
#   "logreg"      -> multinomial Logistic Regression (L2)
#   "linear_svm"  -> Linear SVM (calibrated for predict_proba)
# ======================================================
#CLASSIFIER = "logreg"
...
def get_classifier(name):
    """
    Return a fresh, unfitted simple/linear classifier.
    All return objects expose predict / predict_proba so the same
    evaluation code (including macro-AUC) works unchanged.
    """
    if name == "logreg":
        return LogisticRegression(
            penalty="l2",
            C=1.0,
            class_weight=None,        # oversampling already balances classes
            max_iter=5000,
            multi_class="multinomial",
            solver="lbfgs",
        )
    elif name == "linear_svm":
        # LinearSVC has no predict_proba; calibrate to obtain probabilities
...
# ======================================================
# Train/test balancing (unchanged from GBDT version)
# ======================================================
...
def main():
    ...
    # 3. 5-fold KFold + adjust_train_test + simple classifier
    n_splits = 5
    icycle = 0
